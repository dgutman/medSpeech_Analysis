#!/usr/bin/env python3
"""
Fine-tune Whisper Large model on Medical ASR Dataset
...
def parse_arguments():
    ...
    parser.add_argument('--model-id', type=str, default='openai/whisper-large',
                       help='Whisper model to fine-tune (default: openai/whisper-large)')
    parser.add_argument('--epochs', type=int, default=10,
                       help='Number of training epochs (default: 10)')
    parser.add_argument('--batch-size', type=int, default=4,
                       help='Batch size per GPU (default: 4 for Whisper Large to avoid OOM)')
    parser.add_argument('--learning-rate', type=float, default=0.00001,
                       help='Learning rate (default: 0.00001 for batch size 8)')
...
def main():
    ...
    if is_main_process():
        print("Starting Whisper Large fine-tuning on Medical ASR Dataset...")
    ...
    if out_dir is None:
        # Extract model name from model_id (e.g., "openai/whisper-base" -> "whisper-base")
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        data_seed=42,
        # Memory optimization
        gradient_checkpointing=False,  # Disabled - causes backward pass error with distributed training
