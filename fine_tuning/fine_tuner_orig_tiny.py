#!/usr/bin/env python3
"""
Fine-tune Whisper Tiny model on Medical ASR Dataset
...
def parse_arguments():
    ...
    parser.add_argument('--model-id', type=str, default='openai/whisper-tiny',
                       help='Whisper model to fine-tune (default: openai/whisper-tiny)')
    parser.add_argument('--epochs', type=int, default=10,
                       help='Number of training epochs (default: 10)')
    parser.add_argument('--batch-size', type=int, default=32,
                       help='Batch size per GPU (default: 32 for Whisper Tiny, can be larger than Small)')
    parser.add_argument('--learning-rate', type=float, default=0.00002,
                       help='Learning rate (default: 0.00002 for batch size 32)')
...
def main():
    ...
    if is_main_process():
        print("Starting Whisper Tiny fine-tuning on Medical ASR Dataset...")
    ...
    if is_main_process():
        ...
        # Use explicit cache file names to ensure model-specific caching
        hani89_dataset_train = hani89_dataset_train.map(
            downmix_to_mono,
            num_proc=args.num_proc,
            cache_file_name=f"{out_dir}/cache_train_downmix.arrow"
        )
        hani89_dataset_valid = hani89_dataset_valid.map(
            downmix_to_mono,
            num_proc=args.num_proc,
            cache_file_name=f"{out_dir}/cache_valid_downmix.arrow"
        )
        ...
        # Use explicit cache file names - CRITICAL: tokenizer-specific cache!
        hani89_dataset_train = hani89_dataset_train.map(
            lambda batch: prepare_dataset(batch, feature_extractor, tokenizer),
            num_proc=args.num_proc,
            cache_file_name=f"{out_dir}/cache_train_features.arrow"
        )
        hani89_dataset_valid = hani89_dataset_valid.map(
            lambda batch: prepare_dataset(batch, feature_extractor, tokenizer),
            num_proc=args.num_proc,
            cache_file_name=f"{out_dir}/cache_valid_features.arrow"
        )
    ...
    if not is_main_process():
        ...
        # CRITICAL: Use model-specific cache for tokenized features
        hani89_dataset_train = hani89_dataset_train.map(
            lambda batch: prepare_dataset(batch, feature_extractor, tokenizer),
            num_proc=args.num_proc,
            cache_file_name=f"{out_dir}/cache_train_features.arrow"
