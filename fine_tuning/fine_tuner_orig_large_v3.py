#!/usr/bin/env python3
"""
Fine-tune Whisper Large-v3 model on Medical ASR Dataset
Converted from Jupyter notebook to standalone Python script

Designed for distributed training with torchrun on 4 L40S GPUs.
Whisper Large-v3 is a large model requiring significant GPU memory.
Recommended: Use gradient checkpointing and smaller batch sizes.
"""
...
# Suppress CUDA kernel assertion errors by filtering stderr
import sys
import io

class StderrFilter:
    """Filter stderr to suppress CUDA assertion error messages"""
    def __init__(self, original_stderr):
        self.original_stderr = original_stderr
    
    def write(self, text):
        # Filter out CUDA assertion errors
        if "vectorized_gather_kernel" in text and "index out of bounds" in text:
            return  # Suppress this message
        if "Assertion `ind >=0 && ind < ind_dim_size" in text:
            return  # Suppress this message
...
# Install stderr filter to suppress CUDA assertion errors
...
# Aggressively suppress WhisperTokenizerFast warnings
warnings.filterwarnings("ignore", message=".*WhisperTokenizerFast.*")
warnings.filterwarnings("ignore", message="You're using a WhisperTokenizerFast tokenizer.*")
warnings.filterwarnings("ignore", message=".*Please note that with a fast tokenizer.*")
...
warnings.filterwarnings("ignore", message=".*gemm_and_bias error.*CUBLAS_STATUS_EXECUTION_FAILED.*")
warnings.filterwarnings("ignore", message=".*CUBLAS_STATUS_EXECUTION_FAILED.*")
...
def parse_arguments():
    ...
    parser.add_argument('--model-id', type=str, default='openai/whisper-large-v3',
                       help='Whisper model to fine-tune (default: openai/whisper-large-v3)')
    parser.add_argument('--epochs', type=int, default=10,
                       help='Number of training epochs (default: 10)')
    parser.add_argument('--batch-size', type=int, default=4,
                       help='Batch size per GPU (default: 4 for Whisper Large-v3, adjust based on GPU memory)')
    parser.add_argument('--learning-rate', type=float, default=0.00001,
                       help='Learning rate (default: 0.00001 for Whisper Large-v3)')
    parser.add_argument('--no-cache', action='store_true',
                       help='Disable HuggingFace dataset caching entirely (slower but avoids cache issues)')
    parser.add_argument('--always-clear-cache', action='store_true',
                       help='Always clear cache before training (safer, but slower)')
    parser.add_argument('--no-gradient-checkpointing', action='store_true',
                       help='Disable gradient checkpointing (default: enabled for memory savings)')
    parser.add_argument('--gradient-accumulation-steps', type=int, default=2,
                       help='Gradient accumulation steps (default: 2, increase if you need larger effective batch size or run out of memory)')
...
def clear_cache(out_dir):
    """Clear baseline WER cache (dataset preprocessing cache is handled by HuggingFace)"""
    baseline_cache = f"{out_dir}/baseline_wer_cache.json"
    cleared_files = []
    
    if os.path.exists(baseline_cache):
        os.remove(baseline_cache)
        cleared_files.append(baseline_cache)
        print(f"🗑️  Cleared baseline WER cache: {baseline_cache}")
    else:
        print("ℹ️  No baseline WER cache found to clear")
    
    print("ℹ️  Note: Dataset preprocessing cache is managed by HuggingFace in ~/.cache/huggingface/datasets/")
    print("   To clear dataset cache, delete files in that directory or use HuggingFace's cache management")
...
def setup_wandb(model_id, epochs, batch_size, learning_rate):
...
def prepare_dataset(batch, feature_extractor, tokenizer):
    ...
    for i, tid in enumerate(token_ids):
        # Token IDs must be: -100 (padding), or 0 <= tid < vocab_size
        if tid != -100 and (tid < 0 or tid >= vocab_size):
            invalid_tokens.append((i, tid))
    
    if invalid_tokens:
        raise ValueError(
            f"Invalid token IDs found in batch! "
            f"Vocab size: {vocab_size}, Invalid tokens: {invalid_tokens[:10]} "
            f"(showing first 10). Sentence: {batch.get('sentence', 'N/A')[:100]}"
        )
...
class DataCollatorSpeechSeq2SeqWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        # Validate labels before returning (prevent CUDA assertion errors)
        vocab_size = len(self.processor.tokenizer)
        invalid_mask = (labels != -100) & ((labels < 0) | (labels >= vocab_size))
        if invalid_mask.any():
            invalid_indices = torch.nonzero(invalid_mask, as_tuple=False)
            invalid_values = labels[invalid_mask]
            raise ValueError(
                f"Invalid token IDs in collated batch! "
                f"Vocab size: {vocab_size}, Found {invalid_mask.sum().item()} invalid tokens. "
                f"First few invalid positions: {invalid_indices[:5].tolist()}, "
                f"values: {invalid_values[:10].tolist()}"
            )
...
def main():
    ...
    if is_main_process():
        print("Starting Whisper Large-v3 fine-tuning on Medical ASR Dataset...")
    ...
    # Setup wandb (only on main process) - pass actual config values
    wandb_enabled = False
    if is_main_process():
        wandb_enabled = setup_wandb(model_id, epochs, batch_size, learning_rate)
    
    # Auto-generate output directory from model name if not specified
    if out_dir is None:
        # Extract model name from model_id (e.g., "openai/whisper-tiny" -> "whisper-tiny")
    ...
    if args.always_clear_cache or args.clear_cache:
        if is_main_process():
            print("\n🗑️  Clearing cache as requested...")
            clear_cache(out_dir)
            if args.always_clear_cache:
                print("⚠️  --always-clear-cache: Will disable HuggingFace caching for this run")
    
    # Determine cache behavior
    use_cache = not (args.no_cache or args.always_clear_cache)
    ...
    # Note: HuggingFace datasets automatically caches preprocessing results
    if is_main_process():
        if not use_cache:
            print("⚠️  Caching disabled - preprocessing will be slower but avoids cache issues")
        print("\nPreparing data (downmixing to mono)...")
        map_kwargs = {"num_proc": args.num_proc}
        if not use_cache:
            map_kwargs["keep_in_memory"] = True  # Bypass cache
    ...
    # Gradient checkpointing: Required for Whisper Large-v3 on 48GB GPUs
    # Memory breakdown for Whisper Large-v3 (~1550M params):
    #   - Model weights (bf16): ~3GB
    #   - Optimizer states: ~6GB
    #   - Gradients: ~3GB
    #   - Activations (batch=4): ~20-30GB (can exceed 48GB without checkpointing)
    # Use model-level gradient checkpointing with use_reentrant=False to avoid backward pass errors
    if not args.no_gradient_checkpointing:
        if hasattr(model, 'gradient_checkpointing_enable'):
            try:
                model.gradient_checkpointing_enable(use_reentrant=False)
                if is_main_process():
                    print("✅ Enabled gradient checkpointing on model (use_reentrant=False)")
                    print("   This saves memory by trading compute for memory (slower but fits in 48GB)")
            except Exception as e:
                if is_main_process():
                    print(f"⚠️  Could not enable gradient checkpointing: {e}")
                    print("   Continuing without gradient checkpointing (may run out of memory)")
        else:
            if is_main_process():
                print("⚠️  Model does not support gradient checkpointing")
    else:
        if is_main_process():
            print("⚠️  Gradient checkpointing disabled (may run out of memory with batch size 4)")
    
    # Validate model and tokenizer vocab sizes match (prevent CUDA assertion errors)
    model_vocab_size = model.config.vocab_size
    tokenizer_vocab_size = len(tokenizer)
    if model_vocab_size != tokenizer_vocab_size:
        raise ValueError(
            f"Vocabulary size mismatch! Model vocab size: {model_vocab_size}, "
            f"Tokenizer vocab size: {tokenizer_vocab_size}. "
            f"This will cause CUDA assertion errors during training!"
        )
    if is_main_process():
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        data_seed=42,
        # Memory optimization
        # Note: Gradient checkpointing is enabled on the model directly (see model initialization)
        # Setting it to False here to avoid conflicts with the model-level setting
        # The model-level setting with use_reentrant=False is safer and avoids backward pass errors
        gradient_checkpointing=False,  # Disabled in training args - using model-level instead
